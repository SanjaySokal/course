...
const Students = () => {
    ...
    useEffect(() => {
        document.title = "Students - SoftingArt";
    }, [])

    const getChangeData = (e) => {
        setSearch(e.target.value);
        fetch("https://api.softingart.com/user/student/search", {
            method: "POST",
            headers: {
                        ...
                        <div className="col-lg-8">
                            <form onSubmit={(e) => handleForm(e)}>
                                <label htmlFor="search">
                                    ...
                                    <input type="search" autoComplete="off" onChange={getChangeData} placeholder='Type a keyword' required className='form-control' />
                ...
                <div className="container">
                    <div className="row justify-content-center">
                        {data.map((ele, ind) => <div key={ind} className="col-lg-4 col-md-6 col-sm-12">
                            <Member name={ele.name} desi={ele.desc} link={`/students/${ele.email}`} img={ele.image} socials={ele.socials} />
                        </div>)}
                    </div>
                </div>
            </section>
